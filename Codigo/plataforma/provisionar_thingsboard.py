...
def build_configuration(device_id, catalog):
    ...
    def add(fqn, title, keys, size_x, size_y, row, col, units="", decimals=1,
            icon=None, value_color_fn=None, value_font_size=None,
            label_font_size=None, icon_size=None):
        wid = str(uuid.uuid4())
        ...
        # Cartoes maiores pedem tipografia propria e informam value_font_size.
        if isinstance(settings.get("valueFont"), dict):
            settings["valueFont"]["size"] = value_font_size or 28
        if label_font_size and isinstance(settings.get("labelFont"), dict):
            settings["labelFont"]["size"] = label_font_size
        if icon_size:
            settings["iconSize"] = icon_size
        if icon:
            settings["icon"] = icon
    ...
    add("cards.value_card", "Conexao da Raspberry Pi",
        ...
        value_font_size=56, label_font_size=20, icon_size=56)
